...
const CartItem = ({imgSrc, title, quantity, price, removeAction , id , oldQuantity , oldPrice , oldTotalPrice , totalPrice , currency_id}) => {
    ...
    <div className='bg-[#F8F8F8] py-[30px] px-[15px] flex items-center gap-[30px] rounded-[10px]'>
      {oldPrice?'':
        ...
        <div className="flex-1">
          ...
          {!removeAction && (
            <p className='inline-block xl:hidden text-primary text-[14px] md:text-[18px] font-[700]'>{price} {currency_id}</p>
          )}
        </div>

        <div className='flex items-center gap-[15px]'>
            <p className='hidden xl:inline-block text-primary text-[14px] md:text-[18px] font-[700]'>{oldPrice?<span className='text-danger text-decoration-line-through'>{oldPrice} {currency_id}</span>: price + ' ' + currency_id}</p>
            {
              !removeAction && (
                <div className='rounded-full border border-[#A7A9AC] text-[#575757] flex items-center justify-center gap-[20px] md:gap-[40px] w-[93px] md:w-[134px] p-[5px] md:p-[13px]'>
                    {oldPrice?'':<FaMinus  className='text-[14px] md:text-[20px] cursor-pointer' onClick={()=> countHandler("minus" , id)}/>}
                    <p className='text-dark'><span className='text-danger text-decoration-line-through'>{oldQuantity?oldQuantity:''}</span> {quantity}</p>
                    {oldPrice?'':<FaPlus className='text-[14px] md:text-[20px] cursor-pointer' onClick={()=> countHandler("add" , id)} />}
                </div>
              )
            }
            {console.log(oldPrice , oldQuantity)}
            {
              <>
                <p className='hidden xl:inline-block text-secondery text-[14px] md:text-[18px] font-[700]'>{price} {currency_id}</p>
